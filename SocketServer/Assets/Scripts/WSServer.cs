...
public class WSServer : MonoBehaviour {
	...
	void Update () {
		...
		if (new Regex ("^GET").IsMatch (data)) {
			...
			// normal message
			List<string> newMessages = GetDecodedData (bytes, bytes.Length);
	...
	public static List<string> GetDecodedData(byte[] buffer, int length)
	{
		int msgIndex = 0;
		List<string> messages = new List<string> ();

		while (msgIndex < length - 1) {
			byte b = buffer [msgIndex + 1];
			int dataLength = 0;
			int totalLength = 0;
			int keyIndex = 0;
			...
			if (b - 128 == 126) {
				dataLength = BitConverter.ToInt16 (new byte[] { buffer [msgIndex + 3], buffer [msgIndex + 2] }, 0);
				keyIndex = 4;
				totalLength = dataLength + 8;
			}

			if (b - 128 == 127) {
				dataLength = (int)BitConverter.ToInt64 (new byte[] {
					buffer [msgIndex + 9],
					buffer [msgIndex + 8],
					buffer [msgIndex + 7],
					buffer [msgIndex + 6],
			...
			byte[] key = new byte[] { buffer [msgIndex + keyIndex], buffer [msgIndex + keyIndex + 1], 
				buffer [msgIndex + keyIndex + 2], buffer [msgIndex + keyIndex + 3] };

			int dataIndex = keyIndex + 4;
			int count = 0;
			for (int i = dataIndex; i < totalLength; i++) {
				buffer [i] = (byte)(buffer [msgIndex + i] ^ key [count % 4]);
				count++;
			}

			messages.Add (Encoding.ASCII.GetString (buffer, dataIndex, dataLength));

			msgIndex += totalLength;
		}
